...
function ForgotComponent({sentCode}) {
    ...
    const onSubmitHandler = (e)=>{
        ...
        if(isValid){
            msgRef.current.hide();
            return server.post('/doctor/password/forgot',{identifier:formState.phone}).then((res)=>{
                if(res.data.status){
                    sentCode({
                        id: res.data.id,
        ...
        <div className={style.forgot}>
            <div className={style.wkLoginContent}>
                <div className='text-center mb-3'>
                    <h4 className={style.wpFormHeading}>পাসওয়ার্ড রিকভার করুন </h4>
                </div>
                <hr/>
                ...
                <ul className={style.wkList}>
                    <li><p><span className={style.wkLoginRightSymbol}> <i className="material-icons">
                        done_all
