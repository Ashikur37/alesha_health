...
function ResetComponent({resetData,codeReSent,codeMatched}) {
    ...
    const reSendCode = (e)=>{
        ...
        msgRef.current.hide();
        return server.post('/doctor/password/forgot',{id:resetData.id, identifier:resetData.phone}).then((res)=>{
    ...
    const codeConfirm = (e)=>{
        ...
        msgRef.current.hide();
        return server.post('/doctor/password/code',{identifier:resetData.phone,id:resetData.id,code:code}).then((res)=>{
            setSubmitting(false);
            if(res.data.status){
                codeMatched({
                    code: code
